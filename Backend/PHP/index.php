...
if ($function == "addDeviceToken" && ! empty($DeviceToken) && ! empty($user_id)) {
    $accountObject->addDeviceToken($user_id, $DeviceToken);
}
...
if (! empty($jwtPost)) {
    try {
        ...
        if ($decoded_array['exp'] - $decoded_array['iat'] > 0) {
            ...
            // Logout
            if ($function == "Logout") {
                if ($accountObject->logout($DeviceToken)) {
                    echo "Goodbye";
                }
            }
            ...
            if (! empty($newUsername) && $function == "editUsername") {
                ...
                if ($jsonEditUsername['response'] == 1) {
                    ...
                    $token = [
                        ...
                        'email' => $accountObject->getEmail($user_id),
                        'userID' => $accountObject->getID()
                    ];
            ...
            if (! empty($newEmail) && $function == "editEmail") {
                ...
                echo json_encode($jsonEditEmail);
            }

            if ($function == "deleteAccount" && ! empty($password)) {
                $delJson = array();
                $delJson = $accountObject->deleteAccount($user_id, $password);
                echo json_encode($delJson);
            }
        }
    } catch (exception $e) {
