...
class Account
{
    ...
    {
        $json = array();
        $query = 'DELETE FROM " . $this->user_table . " WHERE account_id = ?';
        if ($stmt = $this->db->getDb()->prepare($query)) {
            $stmt->bind_param('i', $id);
            if ($stmt->execute()) {
                $json['response'] = 1;
    ...
    public function editUsername($id, $NewUsername)
    {
        $json = array();
    ...
    public function editEmail($id, $NewEmail)
    {
        ...
        if (! empty($NewEmail)) {
            ...
            $json['response'] = 0;
            $json['message'] = "login to change email";
        }
        return $json;
    }
    ...
    // will be executed every night at 00:00 using cron
